<script>
    ...
    import Picker from "./Start_end_picker.svelte";
    ...
    */
    ...
    async function loadData(evt) {
        ...
        let start = new Date(evt.detail.start).getTime()/1000
        let stop = new Date(evt.detail.end).getTime()/1000
        console.log(start, stop)
        let server = 'http://132.163.53.82:3201/' // /database/log.db/data?id=${id}&start=${start}&stop=${stop}`)

        let ids = [4, 5, 6, 7, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109]
        /*
        for (let id of ids) {
            console.time('fetch')
            let dataJSON = await fetch(`${server}database/log.db/data?id=${id}&start=${start}&stop=${stop}`)
        ...
        */
        console.time('fetch');
        let dataJSON = await fetch(`${server}database/log.db/data?start=${start}&stop=${stop}`)
        dataJSON = await dataJSON.json();
        console.timeEnd('fetch');
        console.log(dataJSON);
    }
    ...
    onMount( async() => {
        ...
        var responses = await fetch_ids(host, ids, loading_elt, start_ts, end_ts);
...
<div class="body-text">
    ...
    {:else}
        ...
        <Picker startDate = {start_ts} endDate={end_ts} on:done={loadData} />
        <!--
